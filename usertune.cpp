...
bool UserTuneHandler::initConnections(IPluginManager *APluginManager, int &AInitOrder)
{
	...
	if (plugin)
	{
		...
		{
			connect(FRostersViewPlugin->rostersView()->instance(),SIGNAL(indexClipboardMenu(const QList<IRosterIndex *> &, Menu *)),
...
QVariant UserTuneHandler::rosterData(const IRosterIndex *AIndex, int ARole) const
{
	if(ARole == RDR_USERTUNE)
	{
		...
		if (FContactTune[streamJid].contains(senderJid.pBare()))
			return IconStorage::staticStorage(RSR_STORAGE_MENUICONS)->getIcon(MNI_USERTUNE_MUSIC);
	}
	return QVariant();
}
...
void UserTuneHandler::onRosterIndexClipboardMenu(const QList<IRosterIndex *> &AIndexes, Menu *AMenu)
{
...
void UserTuneHandler::setContactTune(const Jid &streamJid, const Jid &senderJid, const UserTuneData &song)
{
	...
	updateDataHolder(senderJid, senderJid);
}
